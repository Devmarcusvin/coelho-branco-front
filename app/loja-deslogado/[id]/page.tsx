...
export default function LojaPageDeslogado() {
  ...
  useEffect(() => {
    ...
    async function carregarDados() {
      ...
      try {
        ...
        console.error("Erro ao carregar dados da loja no modo deslogado:", e);
      } finally {
        setCarregando(false);
      }
    }
      ...
      <div style={{ position: "relative", width: "100%", height: 480, overflow: "hidden" }}>
        ...
        {loja?.usuario?.nome && (
          <div style={{ position: "absolute", bottom: 20, right: 28, color: "#ddd", fontSize: 14, zIndex: 4 }}>
            by <span style={{ textDecoration: "underline", cursor: "pointer" }} onClick={() => router.push("/login")}>
              {loja.usuario.nome}
            </span>
          </div>
        )}
